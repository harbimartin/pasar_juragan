...
@section('tab-content')
    @if (!$detail)
        @if (sizeof($require) > 0)
            <div class="rounded-lg border border-yellow-500 bg-yellow-50 text-yellow-700 py-4 px-6 mt-5 mx-6">
                Perhatian : Untuk dapat melakukan Propose pada Pendaftaran Juragan, pastikan anda telah
                mengunggah/mengaktifkan
                Dokumen
        ...
        <x-add unique="provider" :column="$column_document" title="Tambah Dokumen Juragan" :data="$data" :select="$select"
            idk="id">
        </x-add>
    @endif
    ...
    $table_document = json_encode([
        ...
        'act' => $detail ? null : ['name' => 'Action', 'type' => 'Edit', 'route' => 'dashboard.profile-company.document.edit', 'align' => 'center', 'sort' => false],
    ]);
    ?>
