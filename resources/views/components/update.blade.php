<div class="md:px-6">
    ...
    @endif
    <div class="@if($burl != 'none') pt-20 @else pt-6 @endif">
        <form class="container md:rounded-lg shadow mb-1 md:mb-4 py-2 md:py-4 px-3 md:px-6 bg-white text-xs md:text-base"
                        ...
                        <div @isset($param->if)if="{{ json_encode($param->if) }}"@endisset
                            ...
                            @switch($param->type)
                                    ...
                                    <select id="{{ $key }}" name="{{ $key }}" type="number"
                                        ...
                                        @foreach ($select[$param->api] as $item)
                                            <option value={{ $item['id'] }}
                                                {{ $datas[$param->by] == $item['id'] ? 'selected' : '' }}>
                                ...
                                @case('TextSel')
                                    ...
                                    <input id="{{ $key }}" name="{{ $key }}" value="{{ $datas[$by] }}"
                                        hidden>
                                @break
                                ...
                                @case('Total')
                                    ...
                                    ?>
                                    <input disabled from="{{ json_encode($param->from) }}" id="{{ $key }}"
                                        name="{{ $key }}" value="{{ number_format($def, 2, ',', '.') }}"
                                ...
                                @case('Upload')
                                    ...
                                    <div class="col-end-7 col-start-1 md:col-start-2 flex">
                                        ...
                                        <div v-if="files['{{ $key }}'] && files['{{ $key }}'].length>0"
                                            class="w-full" v-bind:class="{block:true}" hidden>
                                            <div v-for="(file, index) in files['{{ $key }}']"
                                                class="flex w-full py-0.5">
                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                    v-bind:class="file.delete ? 'text-red-800':''" width="25" height="25"
                                                    fill="currentColor" class="my-auto mr-0.5" viewBox="0 0 16 16">
                                                    <path
                                                ...
                                                <input v-bind:disabled="file.delete"
                                                    ...
                                                    v-model="file.name" v-on:input="file.change = file.name!=file.oname"
                                                ...
                                                <label
                                                    ...
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="my-auto" width="16"
                                                    ...
                                                    <span
                                                        class="text-sm my-auto mr-auto font-semibold hidden md:block ml-1">Download</span>
                                                </label>
                                        ...
                                        <span v-else class="my-auto ml-2" hidden v-bind:class="{block:true}">No file
                                            uploaded</span>
                                    </div>
                                @break
                                ...
                                @case('Date')
                                    @if ($detail || isset($param->off))
                                        <input value="{{ date('j F Y', strtotime($datas[$param->by])) }}" type="text"
                                            readonly
                                            class="rounded border col-end-7 col-start-1 md:col-start-2 px-2 py-1 focus:shadow-inner focus:outline-none focus:ring-1 focus:ring-blue-300 focus:border-transparent transition" />
                                    @else
                                        <input id="{{ $key }}" name="{{ $key }}"
                                            value="{{ $datas[$param->by] }}" type="date"
                                            @if ($detail) readonly @endif v-on:change="onlyDate($event)"
                                            class="rounded border col-end-7 col-start-1 md:col-start-2 px-2 py-1 focus:shadow-inner focus:outline-none focus:ring-1 focus:ring-blue-300 focus:border-transparent transition" />
                                    @endif
                                @break
                                ...
                                @case('EmotionScore')
                                    <div class="flex text-3xl md:text-3xl rounded mb-auto">
                                        ...
                                        <label title="Sangat Buruk"
                                            ...
                                            for="es_awfull">😔 <span
                                                class="hidden text-xs my-auto font-semibold text-center">Sangat
                                                Buruk</span></label>
                                        <input type="radio" id="es_bad" name="score" value="2"
                                            @if ($datas[$param->by] == 2) checked @endif hidden>
                                        <label title="Buruk" class="p-1 px-2 md:p-2 pb-2 bg-gray-200 cursor-pointer flex"
                                            for="es_bad">😕
                                        ...
                                        <label title="Biasa" class="p-1 px-2 md:p-2 pb-2 bg-gray-200 cursor-pointer flex"
                                            for="es_normal">😐
                                        ...
                                        <label title="Baik" class="p-1 px-2 md:p-2 pb-2 bg-gray-200 cursor-pointer flex"
                                            for="es_good">🙂
                                        ...
                                        <label title="Sangat Baik"
                                            class="p-1 px-2 md:p-2 pb-2 bg-gray-200 rounded-r-md cursor-pointer flex"
                                            for="es_best">😊 <span
                                                class="hidden text-xs my-auto font-semibold text-center">Sangat
                                                Baik</span></label>
                                    </div>
                                ...
                                @case('Image')
                                    @if ($datas[$key])
                                        <div>
                                            <img src="{{ route('storage', [$param->module, $datas[$key], $id]) }}"
                                                alt="Logo">
                                        </div>
                                    @else
                                ...
                                @case('OpenHour')
                                    ...
                                    @endphp
                                    <div
                                        class="col-start-2 col-end-5 shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                                                        ...
                                                        <td height="50" class="px-3 bg-gray-50 text-gray-500">
                                                            ...
                                                            @if ($open)
                                                                <input hidden
                                                                    ...
                                                                    value="{{ $open['id'] }}">
                                                            @endif
                                                            <input hidden @if ($open) @else disabled @endif
                                                                type="text"
                                                                name="{{ $key }}[{{ $i }}][open_day]"
                                                                value="{{ $value['id'] }}"
                                                                id="inputan-day-{{ $i }}" />
            ...
            <div class="mt-4 md:mt-0">
                <div class="float-right">
                    @if (!$detail)
                        ...
                        <button type="submit" hidden id="_subup_{{ $unique }}_" name="__type"
                            value="update"></button>
                    @endif
                </div>
                {{ $slot }}
