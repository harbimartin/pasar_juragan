<a
    ...
    <div
        class="inline-flex w-full px-3 py-2.5 cursor-pointer {{ isset($on) && $on == $menu['key'] ? 'bg-blue-100 hover:bg-blue-200' : 'hover:bg-gray-100 ' }}">
    ...
    @isset($menu['children'])
        <div id="{{ $menu['key'] }}" name="{{ $menu['key'] }}" class="thidden pl-{{ $margin }}"
            ...
            @foreach ($menu['children'] as $ind => $child)
                <x-sub-menu :menu="$child" :on="$on" :margin="$margin + 3" :level="$level . '_' . $index" :index="$ind">
                </x-sub-menu>
            @endforeach
        </div>
    @endisset
